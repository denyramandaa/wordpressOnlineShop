...
function stOnline_setup() {
	 ...
	 *
	 * Translations can be filed at WordPress.org. See: https://translate.wordpress.org/projects/wp-themes/semi-toko-online
	 * If you're building a theme based on Twenty Twelve, use a find and replace
	 * to change 'semi-toko-online' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'semi-toko-online' );
	...
	register_nav_menu( 'primary', __( 'Primary Menu', 'semi-toko-online' ) );
...
function stOnline_get_font_url() {
	 ...
	 */
	if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'semi-toko-online' ) ) {
		$subsets = 'latin,latin-ext';

		/* translators: To add an additional Open Sans character subset specific to your language,
		 * translate this to 'greek', 'cyrillic' or 'vietnamese'. Do not translate into your own language.
		 */
		$subset = _x( 'no-subset', 'Open Sans font: add new subset (greek, cyrillic, vietnamese)', 'semi-toko-online' );
...
function stOnline_scripts_styles() {
	...
	wp_enqueue_style( 'semi-toko-online-style', get_stylesheet_uri() );
...
function stOnline_wp_title( $title, $sep ) {
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'semi-toko-online' ), max( $paged, $page ) );
...
function stOnline_widgets_init() {
	register_sidebar( array(
		...
		'id' => 'sidebar-1',
		'description' => __( 'Appears on posts and pages except the optional Front Page template, which has its own widgets', 'semi-toko-online' ),
	...
	register_sidebar( array(
		'name' => __( 'First Front Page Widget Area', 'semi-toko-online' ),
		'id' => 'sidebar-2',
		'description' => __( 'Appears when using the optional Front Page template with a page set as Static Front Page', 'semi-toko-online' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => __( 'Second Front Page Widget Area', 'semi-toko-online' ),
		'id' => 'sidebar-3',
		'description' => __( 'Appears when using the optional Front Page template with a page set as Static Front Page', 'semi-toko-online' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
...
function stOnline_content_nav( $html_id ) {
	...
	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="<?php echo esc_attr( $html_id ); ?>" class="navigation" role="navigation">
			<h3 class="assistive-text"><?php _e( 'Post navigation', 'semi-toko-online' ); ?></h3>
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'semi-toko-online' ) ); ?></div>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'semi-toko-online' ) ); ?></div>
		</nav><!-- .navigation -->
	<?php endif;
}
endif;
...
function stOnline_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
		<p><?php _e( 'Pingback:', 'semi-toko-online' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( '(Edit)', 'semi-toko-online' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
			...
			<header class="comment-meta comment-author vcard">
				<?php
					...
					printf( '<cite><b class="fn">%1$s</b> %2$s</cite>',
						...
						( $comment->user_id === $post->post_author ) ? '<span>' . __( 'Post author', 'semi-toko-online' ) . '</span>' : ''
					);
					printf( '<a href="%1$s"><time datetime="%2$s">%3$s</time></a>',
						esc_url( get_comment_link( $comment->comment_ID ) ),
						get_comment_time( 'c' ),
						/* translators: 1: date, 2: time */
						sprintf( __( '%1$s at %2$s', 'semi-toko-online' ), get_comment_date(), get_comment_time() )
					);
				?>
			</header><!-- .comment-meta -->

			<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'semi-toko-online' ); ?></p>
			<?php endif; ?>

			<section class="comment-content comment">
				<?php comment_text(); ?>
				<?php edit_comment_link( __( 'Edit', 'semi-toko-online' ), '<p class="edit-link">', '</p>' ); ?>
			</section><!-- .comment-content -->

			<div class="reply">
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply', 'semi-toko-online' ), 'after' => ' <span>&darr;</span>', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			</div><!-- .reply -->
		</article><!-- #comment-## -->
	<?php
		break;
...
function stOnline_entry_meta() {
	...
	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', __( ', ', 'semi-toko-online' ) );
	...
	$author = sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
		...
		esc_attr( sprintf( __( 'View all posts by %s', 'semi-toko-online' ), get_the_author() ) ),
		get_the_author()
	);

	// Translators: 1 is category, 2 is tag, 3 is the date and 4 is the author's name.
	if ( $tag_list ) {
		$utility_text = __( 'This entry was posted in %1$s and tagged %2$s on %3$s<span class="by-author"> by %4$s</span>.', 'semi-toko-online' );
	} elseif ( $categories_list ) {
		$utility_text = __( 'This entry was posted in %1$s on %3$s<span class="by-author"> by %4$s</span>.', 'semi-toko-online' );
	} else {
		$utility_text = __( 'This entry was posted on %3$s<span class="by-author"> by %4$s</span>.', 'semi-toko-online' );
	}
...
function stOnline_body_class( $classes ) {
	...
	if ( wp_style_is( 'semi-toko-online-fonts', 'queue' ) )
...
require get_template_directory() . '/films/post-type.php';
require get_template_directory() . '/films/taxonomy.php';
require get_template_directory() . '/films/metaboxes.php';
...
// mendapatkan category product
function stOnline_category_films($id)
{
	$category = get_the_terms( $id , 'films_category'); // return array|false|wp_error
 
	if ($category) {
		$list = '';
		foreach ($category as $cat) {
			$url = 'films-category/' . $cat->slug;
			$list .= '<a href="'. home_url($url) .'">'. $cat->name .'</a>';
			$list .= ', ';
		}
...
// mendapatkan tags product
function stOnline_tags_films($id)
{
	$tags = get_the_terms( $id , 'films_tags'); // return array|false|wp_error
 
	if ($tags) {
		$list = '';
		foreach ($tags as $tag) {
			$url = 'films-tags/' . $tag->slug;
			$list .= '<a href="'. home_url($url) .'">'. $tag->name .'</a>';
			$list .= ', ';
		}
